...
class TwitterPreprocessor:
    ...
    def fully_preprocess(self):
        ...
        list_of_words = self.tokenize(return_text)
        return self.stemming(list_of_words)
    ...
    def is_year(self, text):
        if (len(text) == 3 or len(text) == 4) and (self.MIN_YEAR < len(text) < self.MAX_YEAR):
            return True
        else:
            return False
    ...
    def tokenize(self, text):
        ...
        list_of_words = tknzr.tokenize(text)
        return list_of_words

    def stemming(self, list_of_words):
